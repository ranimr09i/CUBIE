...
import glob, os
import re  # !! --- [هام] تم إضافة هذه المكتبة --- !!
...
def story_history(userID: int, request: Request):
    ...
    for r in rows:
        ...
        # تنظيف النص للعرض في القائمة (إزالة التاقات)
        ...
        stories.append({
            ...
            "preferences": r[2],
            "prompt": r[3],
            "generated_story": clean_preview, # نعرض النص النظيف
...
def replay_story(storyID: int, request: Request):
    ...
    for i in range(num_events):
        ...
        # !! --- [المنطق الجديد] استخراج الحركة من النص المخزن --- !!
        move_match = re.search(r"\[(TILTZ|TILTY|SHAKE|FINISH)\]", raw_text)
        
        if move_match:
            saved_move = move_match.group(1)
            # نحذف التاق من النص الذي سيظهر للطفل
            display_text = raw_text.replace(f"[{saved_move}]", "").strip()
        else:
            # في حال القصص القديمة التي لا تحتوي على تاق:
            # إذا كان آخر جزء وفيه كلمة "النهاية" نعتبره FINISH، غير ذلك نفترض TILTZ
            if i == num_events - 1 and ("النهاية" in raw_text or "انتهت" in raw_text):
                saved_move = "FINISH"
            else:
                saved_move = "TILTZ" # حركة افتراضية لإجبار التفاعل
        ...
        events.append({
            "text": display_text,
            "audio_url": audio_url,
            "required_move": saved_move, # نرسل الحركة الحقيقية للتطبيق
