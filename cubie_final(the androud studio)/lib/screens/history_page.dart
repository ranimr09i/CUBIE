...
class _HistoryPageState extends State<HistoryPage> {
  ...
  String _getPreview(String storyText) {
    ...
    final preview = storyText.length > 150
        ? '${storyText.substring(0, 150)}...'
        : storyText;
    return preview.replaceAll('\n', ' ');
  }

  int _getAudioCount(dynamic audioFiles) {
    if (audioFiles == null) return 0;
    if (audioFiles is List) return audioFiles.length;
    return 0;
  }
      ...
      body: RefreshIndicator(
        ...
        child: _isLoading
          ...
          itemBuilder: (c, i) {
            ...
            final date = _formatDate(story['created_at']?.toString() ?? '2025-09-20');
            final preview = _getPreview(story['generated_story']?.toString() ?? '');
            final audioCount = _getAudioCount(story['audio_files']);
            ...
            return Card(
              ...
              child: ListTile(
                ...
                trailing: const Icon(Icons.play_arrow, size: 28, color: Color(0xff4ab0d1)),
                onTap: () {
                  if (story['storyID'] != null) {
                    final storyID = story['storyID'];
                    final storyTitle = "قصة $genre";
                    ...
                    // !! --- (التعديل هنا) --- !!
                    // إرسال إشارة أن هذه قصة من التاريخ
                    Navigator.pushNamed(
                      context,
                      Routes.storyProgress,
                      arguments: {'isReplay': true}, // <-- الإشارة
                    );
                    // !! --- (نهاية التعديل) --- !!
                  } else {
                    ScaffoldMessenger.of(context).showSnackBar(
