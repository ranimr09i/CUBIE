...
// !! --- الإصلاح الوحيد: زيادة الحدود --- !!
const float LIMIT_DEG = 40.0;        // كان 20، صار 40
const float SHAKE_LIMIT_G = 1.2;     // كان 0.7، صار 1.2
const float MIN_THRESHOLD = 15.0;    // حد جديد لتجاهل الحركات الصغيرة
...
// !! --- الإصلاح الوحيد: تحسين دوال الحساس --- !!
...
void detectY() {
  ...
  // تجاهل إذا الميل الجانبي كبير (يعني يلف يمين/يسار مو أمام/خلف)
  if (fabs(angleZ) > MIN_THRESHOLD) {
    Serial.println("Ignoring: Side tilt detected");
    return;
  }
  
  // تجاهل إذا الميل ضعيف
  if (fabs(angleY) < MIN_THRESHOLD) {
    return;
  }
  
  // يحتاج ميل قوي (40 درجة بدلاً من 20)
  if (angleY > LIMIT_DEG) {
    answer = "FORWARD";
    sendBleResponse("GESTURE:FORWARD");
    isQuestionActive = false;
...
void detectZ() {
  ...
  // تجاهل إذا الميل الأمامي/الخلفي كبير (يعني يلف أمام/خلف مو يمين/يسار)
  if (fabs(angleY) > MIN_THRESHOLD) {
...
// bool isPlaying = false;
// bool isPaused = false; // متغير جديد للإيقاف المؤقت
// uint8_t mp3buff[32];      
...
//       isPaused = true;
//       Serial.println("Audio Paused");
//       // إرسال إشعار للتطبيق للتأكيد (اختياري)
//       // sendBleResponse("AUDIO:PAUSED"); 
//     }
//   }
//   else if (command == "RESUME") {
//     if (isPlaying && isPaused) {
...
//   if (fabs(angleZ) > LIMIT_DEG) return; 
...
//   if (fabs(angleY) > LIMIT_DEG) return;
...
//   // !! --- التعديل هنا: التحقق من isPaused --- !!
//   if (isPlaying) {
//     if (client.connected()) {
//       if (client.available() > 0) {
//         // إذا لم يكن موقوفاً، نقرأ البيانات ونشغلها
//         if (!isPaused) {
//           int bytesProcessed = 0;
//           // تقليل البفر قليلاً لزيادة الاستجابة
//           while (client.available() > 0 && musicPlayer.readyForData() && bytesProcessed < 1024) {
//             int bytes = client.read(mp3buff, 32);
//             musicPlayer.playData(mp3buff, bytes);
//             bytesProcessed += bytes;
//           }
//         } else {
//            // إذا كان موقوفاً مؤقتاً، لا نفعل شيئاً ولكن نبقي الاتصال مفتوحاً
//            // (لا تقرأ من العميل حتى لا تستهلك البيانات دون تشغيلها)
//            delay(10); 
//         }
//       }
//     } else {
//       // إذا انقطع الاتصال فعلياً
//       if (isPlaying && !isPaused) { // فقط ننهي إذا لم يكن متوقفاً عمداً (أحياناً البث يتوقف)
//          Serial.println("Stream Ended");
//          stopAudio();
//          sendBleResponse("AUDIO:FINISHED");
//       }
