...
class BluetoothManager extends ChangeNotifier {
  ...
  static const String RESPONSE_CHAR_UUID = "c3856242-4f7f-4a6c-b3d4-4a6e43f5a25c";
  ...
  // --- 1. البحث عن الأجهزة (الكود المُصلح) ---
  Future<void> startScan() async {
    if (_isScanning) return;
    ...
    try {
      print("🔍 [BLE] بدء البحث عن جميع الأجهزة...");

      // !! --- الإصلاح الأساسي --- !!
      // إزالة الفلتر بالـ Service UUID
      await FlutterBluePlus.startScan(
        // withServices: [], // ❌ لا تضع أي فلتر هنا
        timeout: Duration(seconds: 8), // زيادة الوقت قليلاً
      );

      // الاستماع للنتائج
      FlutterBluePlus.scanResults.listen((results) {
        print("📡 [BLE] تم العثور على ${results.length} جهاز");

        // فلترة النتائج لتجنب التكرار
        final uniqueResults = <String, ScanResult>{};
        for (var r in results) {
          // (اختياري) يمكنك فلترة الأجهزة بدون اسم
          // if (r.device.platformName.isEmpty) continue;

          uniqueResults[r.device.remoteId.toString()] = r;

          // طباعة للتشخيص
          if (r.device.platformName.toUpperCase().contains("CUBIE")) {
            print("✅ [BLE] وجدت CUBIE: ${r.device.platformName}");
          }
        }

        _scanResults = uniqueResults.values.toList();

        // ترتيب النتائج: CUBIE أولاً، ثم حسب قوة الإشارة
        _scanResults.sort((a, b) {
          bool aIsCubie = a.device.platformName.toUpperCase().contains("CUBIE");
          bool bIsCubie = b.device.platformName.toUpperCase().contains("CUBIE");

          if (aIsCubie && !bIsCubie) return -1;
          if (!aIsCubie && bIsCubie) return 1;

          return b.rssi.compareTo(a.rssi); // أقوى إشارة أولاً
        });

        notifyListeners();
      });
    } catch (e) {
      print("❌ [BLE] خطأ في البحث: $e");
    } finally {
  ...
  Future<void> stopScan() async {
    ...
    try {
      await FlutterBluePlus.stopScan();
      print("⏹ [BLE] تم إيقاف البحث. النتائج: ${_scanResults.length}");
    } catch (e) {
      print("❌ Error stopping scan: $e");
    }
    _isScanning = false;
  ...
  Future<void> connectToDevice(BluetoothDevice device) async {
    ...
    try {
      print("🔗 [BLE] محاولة الاتصال بـ ${device.platformName}...");

      await device.connect(timeout: Duration(seconds: 15));

      _isConnected = true;
      _cubieDevice = device;
      print("✅ [BLE] تم الاتصال بـ ${device.platformName}");
      notifyListeners();

      await _discoverServices();

      if (!isReady) {
        throw Exception("لم يتم العثور على خدمات CUBIE المطلوبة");
      }

      _connectionStateSubscription = device.connectionState.listen(_onConnectionStateChanged);

      print("✅ [BLE] جاهز للاستخدام!");
      notifyListeners();

    } catch (e) {
      print("❌ [BLE] فشل الاتصال: $e");
  ...
  Future<void> _discoverServices() async {
    ...
    try {
      print("🔍 [BLE] البحث عن الخدمات...");
      List<BluetoothService> services = await _cubieDevice!.discoverServices();

      print("📋 [BLE] تم العثور على ${services.length} خدمة");

      for (var service in services) {
        print("   - Service: ${service.uuid}");

        if (service.uuid == Guid(CUBIE_SERVICE_UUID)) {
          print("✅ [BLE] وجدت خدمة CUBIE!");
          ...
          for (var char in service.characteristics) {
            ...
            if (char.uuid == Guid(COMMAND_CHAR_UUID)) {
              _commandCharacteristic = char;
              print("      ✅ خاصية الأوامر (WRITE)");
            }
            if (char.uuid == Guid(RESPONSE_CHAR_UUID)) {
              _responseCharacteristic = char;
              print("      ✅ خاصية الردود (NOTIFY)");
            }
          }
        }
      }
      ...
      print("❌ [BLE] خطأ في اكتشاف الخدمات: $e");
    }
  }

  // --- 4. إعداد استقبال الردود ---
  Future<void> _setupNotifications() async {
    if (_responseCharacteristic == null || !_isConnected) return;
    ...
    try {
      ...
      print("🔔 [BLE] تم تفعيل استلام الإشعارات.");
    } catch (e) {
      print("❌ [BLE] خطأ في إعداد الإشعارات: $e");
    }
  }
  ...
  Future<void> sendCommand(String command) async {
    if (!isReady) {
      print("⚠ [BLE] غير متصل أو غير جاهز. لا يمكن إرسال: $command");
      return;
    }
    ...
    try {
      ...
      print("❌ [BLE] خطأ أثناء إرسال الأمر: $e");
    }
  }
